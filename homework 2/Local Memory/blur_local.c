...
{   
    ...
    char *programmaKernel = readKernelFile("localPictureProc.cl");
    ...
    // Dimensione dell'array allocato localmente
    int localArraySize = (LOCAL_S+maskDimention-1)*(LOCAL_S+maskDimention-1);
    int localArrayDimention = LOCAL_S+maskDimention-1;
    ...
    err |= clSetKernelArg(kernel, 3, localArraySize, NULL);
    err |= clSetKernelArg(kernel, 4, sizeof(int), (void *)&localArrayDimention);
    err |= clSetKernelArg(kernel, 5, sizeof(int), (void *)&maskDimention);
    err |= clSetKernelArg(kernel, 6, sizeof(int), (void *)&width);
    err |= clSetKernelArg(kernel, 7, sizeof(int), (void *)&height);
    err |= clSetKernelArg(kernel, 8, sizeof(int), (void *)&outWidth);
    err |= clSetKernelArg(kernel, 9, sizeof(int), (void *)&outHeight);
    err |= clSetKernelArg(kernel, 10, sizeof(int), (void *)&ones);
    if (err != CL_SUCCESS)
    {
    ...
    printf("Esecuzione su memoria LOCALE:\n\n");
...
// Cegge il contenuto del file .cl
...
// Crea la mask e ritorna il numero di uni presenti nella mask
